...
def get_monthly_ranking():
    return query_db("SELECT user_id, username, stickers_this_month FROM users WHERE stickers_this_month > 0 ORDER BY stickers_this_month DESC")
...
def register_user_in_group(user_id, chat_id):
    if DATABASE_URL:
        query_db("INSERT INTO group_members (chat_id, user_id) VALUES (?, ?) ON CONFLICT DO NOTHING", (chat_id, user_id))
    else:
...
def get_user_unique_kanto_count(user_id):
    res = query_db("SELECT COUNT(DISTINCT pokemon_id) FROM collection WHERE user_id = ? AND pokemon_id <= 151", (user_id,), one=True)
...
def get_or_create_user(user_id, username):
    ...
    if not res:
        ...
        query_db("INSERT INTO users (user_id, username) VALUES (?, ?)", (user_id, username if username else f"User_{user_id}"))
        
        # --- KIT DE BIENVENIDA AUTOMÁTICO ---
        # 500 Monedas
        add_mail(user_id, 'money', '500', 'Regalo de bienvenida.')
        # 1 Sobre Mediano Nacional
        add_mail(user_id, 'inventory_item', 'pack_medium_national', 'Regalo de bienvenida.')
        # ------------------------------------
...
def mark_event_completed(chat_id, event_id):
    if DATABASE_URL:
        query_db("INSERT INTO group_events (chat_id, event_id) VALUES (?, ?) ON CONFLICT DO NOTHING", (chat_id, event_id))
    else:
